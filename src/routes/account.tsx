...
import { GraduationCap, Plane, ShieldCheck, Clock, CheckCircle2, XCircle, Upload, AlertCircle } from "lucide-react";
      ...
      <div className="container mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8">
        ...
        {isLoading ? (
          ...
          <div className="space-y-6">
              ...
              <div className="rounded-2xl border border-white/10 bg-white/5 p-6">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-white/60">Submit your licence for verification to unlock self-hire.</p>
                  </div>
                </div>
              </div>
            )}
            ...
            {!pending && lastRequest && lastRequest.status !== "approved" && lastRequest.status !== "withdrawn" && (
              <LastDecisionCard request={lastRequest} />
            )}
...
function TierCard({ tier, qualifiedAt }: { tier: string | null; qualifiedAt: string | null }) {
  const meta =
    tier === "pilot"
      ? { Icon: Plane, label: "Pilot", color: "text-emerald-400", bg: "bg-emerald-500/15", blurb: "You're cleared for self-hire bookings." }
      : tier === "student"
      ? { Icon: GraduationCap, label: "Student", color: "text-sky-400", bg: "bg-sky-500/15", blurb: "You can book training lessons with an instructor." }
      : { Icon: ShieldCheck, label: "No tier yet", color: "text-white/70", bg: "bg-white/10", blurb: "Pick a path below to get started." };
      ...
      <div className="flex items-center gap-4">
        ...
        {qualifiedAt && (
          <div className="text-right text-xs text-white/40">
            Qualified <br />{new Date(qualifiedAt).toLocaleDateString("en-GB")}
          </div>
        )}
      </div>
    </section>
...
function ChooseTierCard({ onBecomeStudent, busy, error }: { onBecomeStudent: () => void; busy: boolean; error: string | null }) {
  return (
    <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
      ...
      <div className="mt-4 grid gap-4 sm:grid-cols-2">
        <div className="rounded-xl border border-white/10 bg-white/5 p-5">
          ...
          <p className="mt-2 text-sm text-white/60">Become a student. Instant — no approval needed.</p>
          <button
            disabled={busy}
        ...
        <div className="rounded-xl border border-white/10 bg-white/5 p-5">
          ...
          <p className="mt-2 text-sm text-white/60">Submit your licence below. We'll verify and unlock self-hire.</p>
        </div>
      </div>
    </section>
  );
...
function PendingCard({ request, onWithdraw, busy }: { request: any; onWithdraw: () => void; busy: boolean }) {
  return (
      ...
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1">
          ...
          <p className="mt-1 text-sm text-amber-100/80">
            Submitted {new Date(request.submitted_at).toLocaleString("en-GB")}. We typically review within a few working days.
          </p>
          <dl className="mt-3 grid gap-2 text-sm text-amber-100/80 sm:grid-cols-2">
            <div><dt className="text-xs uppercase tracking-wider text-amber-100/50">Licence</dt><dd className="font-mono">{request.licence_number}</dd></div>
            <div><dt className="text-xs uppercase tracking-wider text-amber-100/50">Authority</dt><dd>{request.issuing_authority}</dd></div>
          </dl>
          <button
            onClick={onWithdraw}
            disabled={busy}
...
function LastDecisionCard({ request }: { request: any }) {
  ...
  return (
    <section className={`rounded-2xl border p-6 ${rejected ? "border-red-500/30 bg-red-500/10" : "border-white/10 bg-white/5"}`}>
      <div className="flex items-start gap-3">
        {rejected ? <XCircle className="mt-1 h-5 w-5 text-red-400" /> : <CheckCircle2 className="mt-1 h-5 w-5 text-emerald-400" />}
        <div>
          <h2 className="text-lg font-bold">{rejected ? "Last request rejected" : `Last request: ${request.status}`}</h2>
          {request.review_notes && <p className="mt-1 text-sm text-white/70">"{request.review_notes}"</p>}
          <p className="mt-2 text-xs text-white/40">
            Reviewed {request.reviewed_at ? new Date(request.reviewed_at).toLocaleString("en-GB") : "—"}. You can submit a new request below.
          </p>
        </div>
      </div>
    </section>
...
function PilotVerificationForm({ onSubmit }: { onSubmit: (payload: any) => Promise<any> }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      setForm({ licence_number: "", issuing_authority: "UK CAA", licence_expiry: "", medical_expiry: "", ratings: "" });
      setLicenceFile(null);
      setMedicalFile(null);
    } catch (err: any) {
  ...
  return (
    <section className="rounded-2xl border border-white/10 bg-white/5 p-6">
      <div className="flex items-center gap-3">
        <ShieldCheck className="h-5 w-5 text-[oklch(0.75_0.18_270)]" />
        <h2 className="text-lg font-bold">Verify as a qualified pilot</h2>
      </div>
      ...
      <form onSubmit={handleSubmit} className="mt-5 grid gap-4 sm:grid-cols-2">
        ...
        <Field label="Licence expiry">
          <input type="date" value={form.licence_expiry} onChange={(e) => setForm((f) => ({ ...f, licence_expiry: e.target.value }))} className={inputCls} />
        </Field>
        <Field label="Medical expiry">
          <input type="date" value={form.medical_expiry} onChange={(e) => setForm((f) => ({ ...f, medical_expiry: e.target.value }))} className={inputCls} />
        </Field>
        ...
        <FileField label="Licence document (PDF or image)" file={licenceFile} setFile={setLicenceFile} />
        <FileField label="Medical document (optional)" file={medicalFile} setFile={setMedicalFile} />

        {error && (
          <div className="sm:col-span-2 flex items-center gap-2 rounded-lg border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-300">
        ...
        <div className="sm:col-span-2 flex justify-end">
          <button
            type="submit"
            disabled={submitting}
            className="inline-flex items-center justify-center gap-2 rounded-xl bg-[oklch(0.55_0.22_270)] px-5 py-2.5 text-sm font-bold text-white hover:brightness-110 disabled:opacity-50"
          >
...
const inputCls = "w-full rounded-lg border border-white/10 bg-white/5 px-3 py-2 text-sm text-white placeholder:text-white/30 focus:border-[oklch(0.55_0.22_270)] focus:outline-none";
