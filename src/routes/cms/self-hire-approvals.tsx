...
function SelfHireAdmin() {
  ...
  const approveMut = useMutation({
    mutationFn: approve,
    onSuccess: () => qc.invalidateQueries({ queryKey: ["self-hire-approvals"] }),
  });
    ...
    <div className="p-8 space-y-6">
      <div>
        ...
        <p className="mt-1 text-sm text-white/50">
          Approve pilots who can book aircraft for self-hire.
        </p>
      </div>
              ...
              return (
                <tr key={a.id} className="border-b border-white/5 text-white">
                  ...
                  <td className="px-4 py-3 text-white/70">
                    {new Date(a.approved_at).toLocaleDateString("en-GB")}
                  </td>
                  <td className="px-4 py-3 text-white/70">
                    {a.expires_at ? new Date(a.expires_at).toLocaleDateString("en-GB") : "—"}
                  </td>
                  ...
                  <td className="px-4 py-3 text-right">
                    {active && (
                      <button
                        onClick={() => {
                          if (window.confirm("Revoke self-hire approval?"))
                            revokeMut.mutate({ data: { user_id: a.user_id } });
                        }}
