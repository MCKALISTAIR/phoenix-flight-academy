...
function ProductsAdmin() {
  ...
  const upsertMut = useMutation({
    mutationFn: upsert,
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["cms-products"] }); setEditing(null); setErr(null); },
    ...
    <div className="p-8 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-white/50">What customers can book — pricing, duration, payment.</p>
        </div>
        <button
          onClick={() => { setEditing({ ...blank }); setErr(null); }}
          className="inline-flex items-center gap-2 rounded-xl bg-[oklch(0.55_0.22_270)] px-4 py-2 text-sm font-bold text-white hover:bg-[oklch(0.60_0.22_270)]"
        >
          ...
          <tbody>
            {(data ?? []).map((p) => (
              <tr key={p.id} className="border-b border-white/5 text-white">
                ...
                <td className="px-4 py-3 text-white/70">{p.requires_approval ? "Required" : "Auto"}</td>
                <td className="px-4 py-3 text-right">
                  <div className="flex justify-end gap-1">
                    ...
                    <button
                      onClick={() => { if (window.confirm(`Delete "${p.name}"?`)) delMut.mutate({ data: { id: p.id } }); }}
      ...
      {editing && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 p-4">
          <div className="max-h-[90vh] w-full max-w-2xl overflow-auto rounded-2xl border border-white/10 bg-[oklch(0.13_0.03_270)] p-6">
            ...
            >
              ...
              <Field label="Kind" req>
                <select value={editing.kind} onChange={(e) => setEditing({ ...editing, kind: e.target.value as ProductForm["kind"] })} className={inputCls}>
              ...
              <Field label="Duration (min)" req>
                <input type="number" value={editing.duration_minutes} onChange={(e) => setEditing({ ...editing, duration_minutes: Number(e.target.value) })} className={inputCls} />
              </Field>
              <Field label="Package price (£)">
                <input type="number" step="0.01" value={editing.package_price_cents != null ? editing.package_price_cents / 100 : ""} onChange={(e) => setEditing({ ...editing, package_price_cents: e.target.value ? Math.round(Number(e.target.value) * 100) : null })} className={inputCls} />
              </Field>
              <Field label="Instructor fee (£/hr)">
                <input type="number" step="0.01" value={editing.instructor_fee_per_hour_cents != null ? editing.instructor_fee_per_hour_cents / 100 : ""} onChange={(e) => setEditing({ ...editing, instructor_fee_per_hour_cents: e.target.value ? Math.round(Number(e.target.value) * 100) : null })} className={inputCls} />
              </Field>
              <Field label="Payment mode" req>
                <select value={editing.payment_mode} onChange={(e) => setEditing({ ...editing, payment_mode: e.target.value as ProductForm["payment_mode"] })} className={inputCls}>
              ...
              <Field label="Deposit %">
                <input type="number" value={editing.deposit_pct} onChange={(e) => setEditing({ ...editing, deposit_pct: Number(e.target.value) })} className={inputCls} />
              </Field>
              <Field label="Cancellation window (hrs)">
                <input type="number" value={editing.cancellation_hours} onChange={(e) => setEditing({ ...editing, cancellation_hours: Number(e.target.value) })} className={inputCls} />
              </Field>
              <Field label="Min notice (hrs)">
                <input type="number" value={editing.min_notice_hours} onChange={(e) => setEditing({ ...editing, min_notice_hours: Number(e.target.value) })} className={inputCls} />
              </Field>
              <Field label="Max advance (days)">
                <input type="number" value={editing.max_advance_days} onChange={(e) => setEditing({ ...editing, max_advance_days: Number(e.target.value) })} className={inputCls} />
              </Field>
              <Field label="Display order">
                <input type="number" value={editing.display_order} onChange={(e) => setEditing({ ...editing, display_order: Number(e.target.value) })} className={inputCls} />
              </Field>
              <Field label="Requires staff approval" full>
                <label className="flex items-center gap-2 text-sm text-white/70">
                  <input type="checkbox" checked={editing.requires_approval} onChange={(e) => setEditing({ ...editing, requires_approval: e.target.checked })} />
              ...
              <div className="sm:col-span-2 mt-2 flex justify-end gap-2">
                <button type="button" onClick={() => setEditing(null)} className="rounded-lg border border-white/10 bg-white/5 px-4 py-2 text-sm text-white/70">Cancel</button>
                <button type="submit" disabled={upsertMut.isPending} className="rounded-lg bg-[oklch(0.55_0.22_270)] px-4 py-2 text-sm font-bold text-white">Save</button>
              </div>
            </form>
          </div>
        </div>
...
function Field({ label, children, req, full }: { label: string; children: React.ReactNode; req?: boolean; full?: boolean }) {
  return (
