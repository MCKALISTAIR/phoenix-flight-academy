  ...
  return (
    <div className="flex h-full min-h-screen">
      <aside className="w-52 shrink-0 border-r border-white/10 bg-black/20 p-4 space-y-1">
        <p className="px-2 pb-3 text-xs font-bold uppercase tracking-widest text-white/30">
          Sections
        </p>
        {sectionKeys.map((key) => {
          const hasDraft =
            !!storedDrafts[key] &&
            JSON.stringify(storedDrafts[key]) !== JSON.stringify(published[key] ?? {});
          ...
          return (
            ...
            >
              ...
              <span className="flex items-center gap-1">
                ...
                {hasLocal && (
                  <span title="Unsaved edits" className="h-2 w-2 rounded-full bg-amber-400" />
                )}
              </span>
            </button>
          );
        ...
        <div className="flex items-center justify-between flex-wrap gap-3">
          <div>
            ...
            <p className="mt-0.5 text-xs text-white/40">
              {Object.keys(display).length} fields ·{" "}
              {viewMode === "published"
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {hasStoredDraft && !hasLocalEdits && (
              <button
                onClick={() => {
                  if (confirm("Discard the pending draft? Staged edits will be lost."))
            ...
            <button
              ...
              disabled={!hasLocalEdits || saveDraftMut.isPending}
            ...
            <button
              onClick={() => {
                if (confirm("Publish this draft to the live site?")) publishMut.mutate();
              }}
              disabled={!draftDiffersFromPublished || publishMut.isPending}
              className="flex items-center gap-1.5 rounded-xl bg-primary px-5 py-2 text-sm font-bold text-white shadow-lg disabled:opacity-40"
            >
        ...
        <div className="rounded-2xl border border-white/10 bg-white/5 divide-y divide-white/5 backdrop-blur-sm overflow-hidden">
          {Object.entries(display).map(([field, value]) => {
            ...
            return (
              <div
                key={field}
                className={`px-6 grid grid-cols-[200px_1fr] gap-4 py-4 ${changed ? "bg-amber-500/5" : ""}`}
              >
...
function HistoryDrawer({
  ...
  return (
    ...
    >
      <div
        className="w-full max-w-3xl h-full bg-primary border-l border-white/10 flex flex-col"
              ...
              return (
                <button
                  ...
                  className={`w-full border-b border-white/5 px-4 py-3 text-left text-xs transition-colors ${
                    active ? "bg-primary/20 text-white" : "text-white/60 hover:bg-white/5"
                  }`}
                >
